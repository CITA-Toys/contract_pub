...
test(`transfer to ${to}`, async () => {
  ...
  const balance = await citaSDK.base.getBalance(to)
  const txRes = await transfer(to, '0x10')
  const receipt = await citaSDK.listeners.listenToTransactionReceipt(txRes.hash)
  if (receipt.errorMessage) {
    throw new Error(errorMessage)
  }
  return await new Promise((resolve, reject) => {
    setTimeout(async () => {
      const balance2 = await citaSDK.base.getBalance(to)
